...
<div class="app-shell" :class="themeClass">
  ...
  <main class="chat-layout">
    ...
    <header class="topbar">
      ...
      <button class="toolbar-btn" @click="moodOpen=!moodOpen" :class="{active: moodOpen}">
        <span class="toolbar-label">90 天情绪记录</span>
...
async function loadSessions(){try{sessions.value=(await getSessionList(username.value)).data||[]}catch(e){flash(e.friendlyMessage||'加载会话失败','error')}}
...
function welcomeHint(title){return{id:crypto.randomUUID(),role:'system',content:`已恢复到会话 **${title||'未命名对话'}**。完整历史会优先从云端加载，本地缓存仅用于加速恢复。`,timestamp:Date.now()}}
...
onMounted(async()=>{const local=hydrateLocal();messages.value=local||[welcome()];resizeTextarea();await loadSessions();await loadMood();const current=sessions.value.find(s=>String(s.sessionId)===sessionId.value);if(current)await openSession(current);bottom()});
